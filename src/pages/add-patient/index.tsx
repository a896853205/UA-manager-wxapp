...
import { PATIENT_DETAIL, PATIENT_SAVE, PATIENT_UPDATE } from '../../constants/api-constants';
...
const AddPatient = () => {
  ...
  const submit = async () => {
    if (!saveDataLoading) {
      ...
      console.log(Taro.getStorageSync('modifyPatient'));

      let res;
      if (Taro.getStorageSync('modifyPatient')) {
        res = await http({
          url: PATIENT_UPDATE,
          method: 'POST',
          data: {
  ...
  return (
    <View>
      <AtForm onSubmit={submit}>
        ...
        <AtButton full formType='submit' type="primary" loading={saveDataLoading}>
          保存
      </AtButton>
      </AtForm>
    </View>
